...
import { buildEvaluationSheets } from "@/lib/evaluation-sheet-data";
import { buildEvaluationSheetsPdfBuffer } from "@/lib/evaluation-sheet-pdf";
...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const sheets = buildEvaluationSheets(survey, responses);
    const buffer = await buildEvaluationSheetsPdfBuffer(sheets);
    const filename = `${safeFilename(survey.title)}-평가지.pdf`;
